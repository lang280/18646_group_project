...
echo -e "${GREEN}Compiling benchmark 'update_weights_kernel_batch'...${NC}"
nvcc -O3 -arch=sm_${compute_capability} --use_fast_math -o update_weights_kernel_batch update_weights_kernel_batch.cu
...
if [ $? -eq 0 ]; then
    ...
    # Run the benchmark
    echo -e "${GREEN}Running benchmark 'update_weights_kernel_batch'...${NC}"
    ./update_weights_kernel_batch "$@"
